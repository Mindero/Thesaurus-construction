...
\begin{frame}
    ...
    \large
    Полнотекстовый поиск не учитывает смысловую близость слов, что снижает точность результатов. 
    
    Семантический поиск позволяет преодолеть эти ограничения. Однако универсальные модели эмбеддингов, обученные на общих корпусах, не улавливают лексико"=семантические особенности конкретной предметной области.

    \textbf{Цель работы}: разработать систему семантического поиска по корпусу новостных документов на основе эмбеддингов.
\end{frame}

\begin{frame}
    \frametitle{Используемые технологии \slnumber}
    \large
    \begin{itemize}
        \item Python $3.12.6$
        \begin{itemize}
...
\begin{frame}
  ...
  \large
  В качестве корпуса новостных документов были взяты новости СГУ за период с $2007$ по $2022$ года.

  Обработка текста заключалась в следующих этапах:
  \begin{enumerate}
...
\begin{frame}
  ...
  \large
  Были обучены на обработанных текстах новостей модели Word2Vec и Glove со следующими параметрами:
  \begin{itemize}
...
\begin{frame}
  \frametitle{Вывод по анализу моделей, построенных на основе корпуса новостных документов \slnumber}
  \large
...
\begin{frame}
  ...
  \textbf{Решение.} Была взята модель \texttt{ruscorpora\_upos\_skipgram\_300\_5\_2018} и обученные модели Word2Vec и GloVe расширены по следующему алгоритму:
  \begin{enumerate}
...
\begin{frame}
  ...
  \begin{enumerate}
    \item \textbf{Предварительная обработка запроса.} Происходит удаление знаков препинания и имён собственных, лемматизация.
    \item \textbf{Получение эмбеддинга запроса.} Вектор запроса формируется как среднее арифметическое всех векторов слов запроса.
    \item \textbf{Поиск релевантных документов.} Поиск осуществляется с помощью алгоритма ближайших соседей. Результатом этого этапа является список из $k$ идентификаторов новостей.
    \item \textbf{Вывод результата.} По идентификаторам вычисляются заголовки новостей.
  \end{enumerate}
\end{frame}
...
\begin{frame}
  ...
  Результаты поэтапно:
  \begin{enumerate}
      \item Результат предварительной обработки: ['поздравить', 'сгу', 'праздник', 'победа']
      \item Получаемый эмбеддинг запроса: 
      
      [[ 0.1731391   0.4556614   0.21552482  0.66046524, ...]]
      \item По эмбеддингу найдены 5 ID ближайших документов: 
...
\begin{frame}
  ...
  \large
  Для анализа создана тестовая выборка из пар: запрос — идентификатор новости. Cчитается, что модель успешно ответила на запрос, если релевантная новость находится в топ"=$k$ результатов.
